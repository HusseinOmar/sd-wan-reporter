...
# Maximum number of concurrent SD-WAN Manager API calls performed by the backup
# task. Kept moderate to speed up collection without overwhelming the manager.
...
class TaskBackup(Task):
    ...
    def runner(self, parsed_args, api: Optional[Rest] = None) -> Sequence | None:
        ...
        for _, info, index_cls, item_cls in catalog_iter(*parsed_args.tags, version=api.server_version):
            ...
            def backup_item(item_id, item_name, info=info, item_cls=item_cls, item_index=item_index):
                try:
            ...
            # Fetch the items of this tag concurrently (bounded), preserving identical output files.
            with ThreadPoolExecutor(max_workers=BACKUP_MAX_WORKERS) as executor:
                for item_id, item_name in matched_item_iter:
                    executor.submit(backup_item, item_id, item_name)
    ...
    def save_running_configs(self, api: Rest, workdir: str) -> None:
        ...
        def save_device_config(uuid, hostname):
            ...
            for inventory, info in inventory_list:
                ...
                for uuid, _, hostname, _ in inventory.extended_iter():
                    if hostname is None:
                        ...
                        continue
                    executor.submit(save_device_config, uuid, hostname)
